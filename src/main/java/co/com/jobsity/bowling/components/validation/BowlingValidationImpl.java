...
public class BowlingValidationImpl implements BowlingValidation {
    ...
    protected static final String GAME_FRAMES_MSG = "Game is not valid due we only support "
            + BowlingPinfallConstants.MAX_FRAME + " frames.";
    protected static final String VALID_FRAME_MSG = "Game is not valid due we only support "
            + BowlingPinfallConstants.MAX_PIN + " pin games.";
    protected static final String VALID_FRAME_LAST_FRAME_MSG = "Third roll is only allowed when you wipe at least "
            + BowlingPinfallConstants.MAX_PIN + " pins";
    ...
    public boolean isFrameFinished(BowlingFrame frame) {
        ...
        return extractBonusFromFrame(frame) ? falls.size() == 3 : falls.size() == 2;
    }
