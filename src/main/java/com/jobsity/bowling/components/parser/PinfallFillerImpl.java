...
public class PinfallFillerImpl implements PinfallFiller {
    ...
    Pinfall getPinfallForSpare(BowlingFrame frame, String score, int lastScore) {
        ...
        if (bowlingValidation.isSpare(sum)) {
            ...
            pinfall =
                    Pinfall.builder()
                            .fall(newScore)
                            .fallValue(BowlingPinfallConstants.SPARE)
                            .build();
        } else {
    ...
    void addPinfallOnLastRoundWhenNotStrike(BowlingFrame frame, String score) {
        if (frame.getPinfalls().isEmpty()) {
            frame.getPinfalls()
                    .add(Pinfall.builder().fallValue(score).fall(getFallScore(score)).build());
        } else if (frame.getPinfalls().size() == 1) {
            int lastRollScore =
                    frame.getPinfalls().stream().findFirst().map(Pinfall::getFall).orElse(0);
            frame.getPinfalls().add(getPinfallForSpare(frame, score, lastRollScore));
        } else {
    ...
    void addPinfallOnLastFrameRollThree(BowlingFrame frame, String score) {
        int firstAttempt =
                Optional.ofNullable(frame.getPinfalls().get(0)).map(Pinfall::getFall).orElse(0);
        int secondAttempt =
                Optional.ofNullable(frame.getPinfalls().get(1)).map(Pinfall::getFall).orElse(0);
